<?php
	...
	$title_style = onepagepro_get_option('general', 'default-title-style', 'small');
	$title_align = onepagepro_get_option('general', 'default-title-align', 'left');

	// for index page
	if( is_home() ){
		$main_head = true;
		$page_title = esc_html__('Blog', 'onepagepro');
	...
	// search page
		...
		if( have_posts() ){
			$page_title = esc_html__('Search Results', 'onepagepro');
	...
	// archive page
		...
		if( is_category() || is_tax('portfolio_category') || is_tax('product_cat') ){
			$page_title = esc_html__('Category', 'onepagepro');
			...
			$page_title = esc_html__('Tag', 'onepagepro');
			...
			$page_title = esc_html__('Day','onepagepro');
			$page_caption = get_the_date('F j, Y');
		}else if( is_month() ){
			$page_title = esc_html__('Month','onepagepro');
			$page_caption = get_the_date('F Y');
		}else if( is_year() ){
			$page_title = esc_html__('Year','onepagepro');
			$page_caption = get_the_date('Y');
		}else if( is_author() ){
			$page_title = esc_html__('By','onepagepro');
			
			$author_id = get_query_var('author');
			$author = get_user_by('id', $author_id);
			$page_caption = $author->display_name;					
		}else if( is_post_type_archive('product') ){
			$page_title = esc_html__('Shop', 'onepagepro');
			$page_caption = '';
		}else{
			$page_title = get_the_title();
			$page_caption = '';
	...
	if( (empty($post_option['enable-page-title']) || $post_option['enable-page-title'] == 'enable') && (!empty($page_title) || !empty($page_caption)) ){	
		...
		$page_title = apply_filters('onepagepro_page_title', $page_title);
		$page_caption = apply_filters('onepagepro_page_caption', $page_caption);

		$extra_class  = ' onepagepro-style-' . $title_style;
		$extra_class .= ' onepagepro-' . $title_align . '-align';

		if( empty($post_option['title-background-type']) || $post_option['title-background-type'] == 'default' ){
			$page_title_background = onepagepro_get_option('general', 'default-title-background-style', 'image');
			if( $page_title_background == 'none' ){
				$extra_class .= ' onepagepro-without-background';
			}
		}else if($post_option['title-background-type'] == 'none'){
			$title_background = '';
			$extra_class .= ' onepagepro-without-background';
		}


		echo '<div class="onepagepro-page-title-wrap ' . esc_attr($extra_class) . '" ' . gdlr_core_esc_style(array(
			'background-image' => empty($title_background)? '': $title_background
		)) . '>';
		echo '<div class="onepagepro-header-transparent-substitute" ></div>';
		echo '<div class="onepagepro-page-title-overlay" ' . gdlr_core_esc_style(array(
		...
		echo '<div class="onepagepro-page-title-container onepagepro-container" >';
		echo '<div class="onepagepro-page-title-content onepagepro-item-pdlr" ' . gdlr_core_esc_style(array(
		...
		if( !empty($page_title) ){
			if( !empty($main_head) ){
				echo '<h1 class="onepagepro-page-title" ' . gdlr_core_esc_style(array(
				...
				echo '<h3 class="onepagepro-page-title" ' . gdlr_core_esc_style(array(
					'font-size' => empty($title_size['title-size'])? '': $title_size['title-size'],
					'font-weight' => empty($title_weight['title-weight'])? '': $title_weight['title-weight'],
					'text-transform' => (empty($title_transform) || $title_transform == 'default')? '': $title_transform,
					'letter-spacing' => empty($title_size['title-letter-spacing'])? '': $title_size['title-letter-spacing'],
		...
		if( !empty($page_caption) ){
			echo '<div class="onepagepro-page-caption" ' . gdlr_core_esc_style(array(
		...
		echo '</div>'; // onepagepro-page-title-content
		echo '</div>'; // onepagepro-page-title-container
		echo '</div>'; // onepagepro-page-title-wrap
	}
?>